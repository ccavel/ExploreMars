<?php
require_once "includes/navbar.php"; 
?>

<!doctype html>
<html>
<body>

<br/>
<br/>
<div class="container text-center">
<div class="jumbotron jumbotron-fluid bg-white">
<div class="container">
		<h1>Téléchargements</h1>
		<p>Ici, vous pourrez télécharger l'ensemble de nos documents.</p> <br/>
		
		<table class="table table-danger">
		<tr>
			<th>Livrable</th><th>Description</th><th>Poids</th><th>Télécharger</th>		
		</tr>
		<tr>
			<td>Cahier des charges</td>
			<td>Décrit les objectifs du premier semestre ainsi que les spécifications techniques du manuel utilisateur</td>
			<td>0,5 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/CdC.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>
	
		</tr>
		<tr>
			<td>Etat de l'art</td>
			<td>Bibliographie explicitant les études réalisées sur les sujets de la Conscience de Situation et du Partage de Conscience de Situation</td>
			<td>1,3 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/EdA.pdf"><img src="images/upload.png"  alt="7th slide" width=30%></a>
			</td>				
		</tr>
		<tr>
			<td>Protocole</td>
			<td>Document détaillant le protocole (matériel, déroulement) de l'expérience effectuée</td>
			<td>0,1 Mo</td>
			<td width=10% class="text-center">
			<a  href="downloads/Protocole.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>				
		</tr>
		<tr>
		<td>Questionnaire</td>
			<td>Questionnaire post-expérimental rempli online par chacun des participants et complété par leur(s) examinateur/trice(s)</td>
			<td>0,2 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/Questionnaire.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>				
		</tr>
		<tr>
		<td>Données brutes</td>
			<td>Dossier contenant toutes les données brutes récoltées post-expérience</td>
			<td>50 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/Questionnaire.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>				
		</tr>

		</table> 
		
		<br/>
		<br/>

		<table class="table table-danger">
		<tr>
			<th>Article</th><th>Auteurs</th><th>Description</th><th>Poids</th><th>Télécharger</th>		
		</tr>
		<tr>
			<td>Shared Spatial Situation Awareness as a Team Performance Indicator in Collaborative Spatial Orientation Task (AHFE)</td>
			<td>Baptiste Prebot, Jean-Marc Salotti, Coralie Vennin, Bernard Claverie</td>
			<td>Article étudiant le lien entre la performance temporelle d'une équipe nécessaire à la réalisation d'une mission d'orientation spatial
			et une évaluation du partage de conscience de situation entre les membres de l'équipe </td>
			<td>1,2 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/AHFE.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>
	
		</tr>
		<tr>
			<td>Supporting situation awareness on the move - the role of technology for spatial orientation in the field</td>
			<td>Björn J.E. Johansson, Charlotte Hellgren, Per-Anders Oskarsson, Jonathan Svensson</td>
			<td>Etude interrogeant comment la technologie peut améliorer la conscience de situation lors
			d'une mission de déplaement dans un terrain boisé. La capacité à retrouver un objet sur le terrain à une 
			grande importance lors des opérations d'urgences et de recherches de survivants.</td>
			<td>0,4 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/SupportingSAonTheMove.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>
		</tr>
		<tr>
			<td>AMADEE-20 Announcement Of Oppotunity</td>
			<td>OWF (ÖSTERREICHISCHES WELTRAUM FORUM) and AUSTRIAN SPACE FORUM</td>
			<td>Appel à expérience scientifique sur le thème de la recherche spatiale. Une exprience dans la continuité de celle réalisée cette année dans
			ce transdiciplinaire sera réalisée, dans un environnement externe simulant un environnement martien. Il s'agira cette fois de donner toutes les directives
			à Astro en avance et d'une seule traite. Ce dernier devra alors aller sur le terrain (en combinaison) à la recherche de la roche, sans aucun moyen de communication avec CapCom</td>
			<td>2 Mo</td>
			<td width=10% class="text-center">
			<a href="downloads/OWF.pdf"><img src="images/upload.png" alt="7th slide" width=30%></a>
			</td>
		</tr>

</table>
<p class="lead text-center">
Un dossier contenant la simulation sous forme d’exécutable Unity et le logiciel de pointage de carte a été remis au client 
et au tuteur. Il ne sera pas publié ici pour des raisons de confidentialités.
</p>
		</div>
		</div>

</div>
	
    <?php require_once "includes/footer.php"; ?>

</body>

</html>
